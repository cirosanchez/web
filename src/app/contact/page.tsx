export default function ContactPage() {
    return (
        <div className="text-neutral-400 flex justify-center">
            <div className="max-w-2xl w-full pt-4 pb-10">
                <div className="leading-relaxed space-y-6 pb-4 text-lg text-neutral-300">
                    <strong>Contact Me</strong>
                </div>
                <div className="text-sm leading-relaxed space-y-6">
                    <p>
                        Feel free to reach out to me via email at <a className="text-neutral-300 hover:underline" href="mailto:ciro.sanchez@ucalgary.ca">ciro.sanchez@ucalgary.ca</a>
                    </p>
                </div>
            </div>
        </div>
    );
}
